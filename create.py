...
#  2022/9/4     新增加一个详细说明功能 选择桌游后打开一个对应的txt文档里面会有开场介绍规则简述和内容清单
...
class Boardgame(object):
    def __init__(self, name, en_name, player_number, game_type, time, weight, state, intro):
        self.name = name
...
class BoardgameManager(object):
    ...
    def start(self):
        ...
        for msg in storage_input:
            ...
            if n % 9 == 0:  # 一共有八项内容 所以被九整除后 往中列表里添加小列表
        ...
        for small_list in mid_list:
            a = Boardgame(small_list[0], small_list[1], small_list[2], small_list[3],
                          small_list[4], small_list[5], small_list[6], small_list[7])
    ...
    def add_boardgame(self):
        ...
        if name and input(f'你想输入的是{name}吗，按y确认，如果不是请输入任意内容或直接回车继续完成添加') != 'y':
            name = name1
        for boardgame in self.boardgame_list:
        ...
        if a:
            ...
            intro = input('简介\n')
            self.boardgame_list.append(Boardgame(name, en_name, player_number, game_type, time, weight, state, intro))
            print(f'{name}添加成功')
    ...
    def match_boardgame(self, game_type, player_number, difficulty):
        ...
        if game_type == '都行':
            for Boardgame1 in self.boardgame_list:
                if len(Boardgame1.player_number) == 1:
                    n = 1
                else:
        ...
        for Boardgame1 in self.boardgame_list:
            if Boardgame1.game_type == f'{game_type}':
                if len(Boardgame1.player_number) == 1:
                    n = 1
                else:
                    a, b = Boardgame1.player_number.split('-')
    ...
    def txt_boardgame(self, name):
        ...
        txt_file = open(f'txt/{file_name}.txt', mode='r')
        print(txt_file.read())

    def create(self):
        for boardgame in self.boardgame_list:
            file_name = boardgame.name
            file_create = open(f'txt/{file_name}.txt', mode='w')
            file_create.close()
